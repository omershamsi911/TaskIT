...
const PasswordStrength = ({ password }) => {
  const getStrength = () => {
    if (!password) return { level: 0, label: "", color: T.LIGHT_IK };
    let score = 0;
    ...
    if (score <= 1) return { level: 1, label: "WEAK", color: T.RED };
    if (score <= 3) return { level: 2, label: "FAIR", color: "#F5A623" };
    return { level: 3, label: "STRONG", color: T.GREEN };
  };
        ...
        {[1, 2, 3].map((i) => (
          <div
            ...
            style={{
              flex: 1,
              height: 3,
              background: i <= strength.level ? strength.color : `${T.IK}20`,
              transition: "background 0.2s",
            }}
          />
        ))}
      </div>
      <span style={{ fontSize: 8, fontWeight: 900, letterSpacing: "0.1em", color: strength.color }}>
        {strength.label}
      </span>
    </div>
  );
...
const Register = () => {
  ...
  const fields = [
    { name: "full_name", label: "FULL NAME", type: "text", placeholder: "YOUR FULL NAME", icon: (
    ...
    { name: "phone", label: "PHONE NUMBER", type: "tel", placeholder: "0300 0000000", icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div
      style={{
        ...
        padding: isMobile ? "24px 16px 48px" : "24px 24px 48px",
        position: "relative",
            ...
            {["customer", "provider"].map((r) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  border: "none",
                  borderRight: r === "customer" ? `1px solid ${T.IK}` : "none",
                  background: formData.role === r ? T.C : "transparent",
                  color: formData.role === r ? T.CR : T.IK,
            ...
            <div key={f.name} style={{ marginBottom: i === fields.length - 1 ? 32 : 24 }}>
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  style={{
                    width: "100%",
                    padding: f.name === "password" ? "14px 44px 14px 44px" : "14px 16px 14px 44px",
                    fontSize: 10,
                    fontWeight: 900,
                    letterSpacing: "0.12em",
                    textTransform: f.type !== "password" ? "uppercase" : "none",
                    background: "transparent",
                    border: `1px solid ${focused === f.name ? T.C : T.IK}`,
                    outline: "none",
                    color: T.IK,
                    transition: "border-color 0.15s, box-shadow 0.15s",
                    boxSizing: "border-box",
                    boxShadow: focused === f.name ? `0 0 0 3px ${T.C}15` : "none",
                  }}
                />
                <div
                  style={{
                    ...
                    color: focused === f.name ? T.C : T.LIGHT_IK,
              ...
              {f.name === "password" && <PasswordStrength password={formData.password} />}
            </div>
          ))}
